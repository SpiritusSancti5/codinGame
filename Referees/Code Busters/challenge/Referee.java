...
class Referee extends MultiReferee {
	...
	public static int[] GHOST_STAMINA = { 3, 15, 40 };
	...
	public static boolean CAN_STUN = true;
	...
	protected String[] getInputForPlayer(int round, int playerIdx) {
		...
		for (int j = 0; j < players.size(); ++j) {
			...
			for (Buster enemy : players.get(j).busters) {
				for (Buster b : player.busters) {
					if (enemy.position.distance(b.position) <= VIEW_RANGE) {
		...
		for (Ghost ghost : ghosts) {
			for (Buster b : player.busters) {
				if (!ghost.inHq && !ghost.isGrabbed() && ghost.position.distance(b.position) <= VIEW_RANGE) {
		...
		for (Obstacle o : map) {
			for (Buster b : player.busters) {
				if (o.position.distance(b.position) <= VIEW_RANGE + o.getRadius()) {
				...
				if (matchMove.matches()) {
					...
					if (buster.position.getX() != x || buster.position.getY() != y) {
						...
						if (v.lengthSquared() <= speed * speed) {
							...
							buster.movement = v.normalize().mult(speed);
						}
					...
					buster.grabTarget = id;
					buster.attemptGrab = true;
					...
					if (buster.state == Buster.CARRYING) {
						buster.release = buster.grabTarget;
						buster.state = Buster.IDLE;
						buster.grabTarget = null;
					} else {
					...
					buster.stunTarget = id;

					// Message
					matchMessage(buster, matchStun);
				} else {
					throw new InvalidInputException("MOVE x y | BUST id | RELEASE" + (CAN_STUN ? " | STUN id" : ""), line);
				}
			} catch (Exception e) {
	...
	protected void updateGame(int round) throws GameOverException {
		...
		for (Buster buster : allBusters) {
			...
			if (move != null) {
				...
				if (!collidesWith.isEmpty()) {
					...
					newPos = buster.position.add(v.mult(smallest));
				}

				buster.prevPosition = buster.position;
				// buster.position = newPos;
				buster.nextPosition = newPos;
			} else if (buster.attemptGrab && buster.grabTarget != null) {
				Ghost target = null;
				if (buster.grabTarget >= 0 && buster.grabTarget < ghosts.size()) {
					target = ghosts.get(buster.grabTarget);
			...
			} else if (buster.stunTarget != null) {
				if (buster.state == Buster.GRABBING) {
					buster.state = Buster.IDLE;
				}
				Buster target = null;
				if (buster.stunRecharge == 0 && buster.stunTarget >= 0 && buster.stunTarget < allBusters.size()) {
					target = allBusters.get(buster.stunTarget);
				...
				if (target == null) {
					...
					buster.stunSuccess = true;
					buster.stunRecharge = STUN_COOLDOWN;
					buster.letGo();
					stunMap.put(buster, target);
				}
			} else if (buster.state == Buster.GRABBING) {
				buster.state = Buster.IDLE;
				buster.grabTarget = null;
			}
		...
		for (Buster buster : allBusters) {
			if (buster.nextPosition != null) {
				...
				buster.rotation = new Vector(buster.prevPosition, buster.position).angle();
			}
		...
		for (Ghost ghost : ghosts) {
			...
			if (ghost.isGrabbed()) {
				if (ghost.grabbedBy.state != Buster.CARRYING || ghost.grabbedBy.grabTarget != ghost.id) {
					if (ghost.inHq) {
						ghost.grabbedBy.owner.lastCaptureFrame = round;
					}
		...
		for (Entry<Buster, Buster> stunEntry : stunMap.entrySet()) {
			...
			if (stunned.state == Buster.CARRYING) {
				if (stunned.grabTarget >= 0 && stunned.grabTarget < ghosts.size()) {
					Ghost ghost = ghosts.get(stunned.grabTarget);
					if (ghost.grabbedBy == stunned) {
						ghost.grabbedBy = null;
						ghost.flagUpdated();
					}
				}
	...
	protected List<String> getPlayerSummary(int playerIdx) {
		...
		for (Buster b : player.busters) {
			...
			} else if (b.movement != null) {
				if (b.state == Buster.CARRYING) {
					lines.add(translate("moveAndBust", b.id, player.index, (int) b.position.getX(), (int) b.position.getY(), b.grabTarget));
				} else {
					lines.add(translate("move", b.id, player.index, (int) b.position.getX(), (int) b.position.getY()));
				}
			} else if (b.stunTarget != null) {
				if (b.stunSuccess) {
					if (b.stunTarget.intValue() == b.id)
						lines.add(translate("autostun", b.id, player.index));
					else
						lines.add(translate("stun", b.id, player.index, b.stunTarget, allBusters.get(b.stunTarget).owner.index));
				} else if (b.stunRecharge > 0) {
					lines.add(translate("stunErrorRecharge", b.id, player.index, b.stunTarget));
				} else {
					lines.add(translate("stunError", b.id, player.index, b.stunTarget));
				}
			} else if (b.state == Buster.GRABBING) {
				lines.add(translate("busting", b.id, player.index, b.grabTarget));
			} else if (b.grabSuccess()) {
				lines.add(translate("bust", b.id, player.index, b.grabTarget));
			} else if (b.release != null) {
				lines.add(translate("release", b.id, player.index, b.release));
			} else if (b.attemptGrab && b.grabTarget != null) {
				if (b.tooClose) {
					lines.add(translate("bustTooClose", b.id, player.index, b.grabTarget));
				} else {
					lines.add(translate("bustError", b.id, player.index, b.grabTarget));
				}
...
class Ghost extends GameEntity {
	...
	public boolean isTrappedBy(PlayerInfo player) {
		return isGrabbed() && grabbedBy.grabTarget != null && grabbedBy.grabTarget.equals(id) && grabbedBy.state == Buster.CARRYING && grabbedBy.owner == player;
	}
...
class Buster extends GameEntity {
	...
	int index, stunCountDown, stunRecharge;
	PlayerInfo owner;
	String message;
	Vector movement;
	Vector prevPosition, nextPosition;
	double rotation;
	Integer grabTarget, stunTarget, release;
	boolean attemptGrab, confused, stunSuccess, finishGrab, tooClose;
	...
	public String toFrameString() {
		return position.toIntString() + " " + state + " " + (int) (rotation * 180 / Math.PI) + " " + (confused ? 1 : 0) + " " + (stunSuccess ? stunTarget : -1) + " " + ((state == GRABBING || finishGrab) ? grabTarget : -1) + " " + (message != null ? message : "");
	}
	...
	public boolean grabSuccess() {
		return attemptGrab && state == Buster.CARRYING;
	}
