...
class Referee extends MultiReferee {
    ...
    private static int OBLIVIATE_DURATION = 4;
    ...
    private static int ACCIO_MAGIC = 15;
    ...
    class Bludger extends DynamicEntity {
        ...
        @Override
        public String toPlayerString() {
            return this.id + " " + this.type + " " + Math.round(position.x) + " " + Math.round(position.y) + " " + Referee.symmetricRound(speed.x) + " " + Referee.symmetricRound(speed.y) + " " + (lastVictim != null ? lastVictim.id : -1);
        }
    ...
    class Snaffle extends DynamicEntity {
        ...
        @Override
        public String toPlayerString() {
            return this.id + " " + this.type + " " + Math.round(position.x) + " " + Math.round(position.y) + " " + Referee.symmetricRound(speed.x) + " " + Referee.symmetricRound(speed.y) + " " + (this.captured ? 1 : 0);
        }
    ...
    protected String[] getInputForPlayer(int round, int playerIdx) {
        ...
        Player player = this.players[playerIdx];
        Player opponent = this.players[(playerIdx + 1) % 2];
        
        data.add(player.score + " " + player.magic);
        data.add(opponent.score + " " + opponent.magic);
        
        for (Pod p : this.pods) {
            if (p.player == player) {
                dataEntities.add(p.toPlayerString(true));
            }
        }

        for (Pod p : this.pods) {
            if (p.player != player) {
                dataEntities.add(p.toPlayerString(false));
            }
        }

        for (Snaffle snaffle : this.snaffles) {
            if (snaffle.alive) {
                dataEntities.add(snaffle.toPlayerString());
            }
        }
        for (Bludger bludger : this.bludgers) {
            dataEntities.add(bludger.toPlayerString());
        }

        dataEntities.add(0, dataEntities.size() + "");

        data.addAll(dataEntities);
        
        return data.toArray(new String[dataEntities.size()]);
    }
